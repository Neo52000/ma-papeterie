...
Deno.serve(async (req) => {
  ...
  // ALKOR_B2B crawl now uses Playwright via GitHub Actions (sync-alkor.yml)
  // The edge function cannot run a headless browser, so redirect to the proper path
  let sessionCookie = "";
  if (job.source === "ALKOR_B2B") {
    console.log("run-crawl: ALKOR_B2B crawl redirected to GitHub Actions (Playwright)");
    await supabase.from("crawl_jobs").update({
      status: "error",
      last_error: "Le crawl ALKOR B2B utilise désormais GitHub Actions avec Playwright. " +
        "Utilisez le bouton 'Lancer le crawl B2B' dans l'onglet Sync B2B de la page Admin Alkor.",
    }).eq("id", jobId);
    return new Response(
      JSON.stringify({ error: "Le crawl ALKOR B2B utilise désormais GitHub Actions. Utilisez l'onglet Sync B2B." }),
      { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }
