...
Deno.serve(createHandler({
  name: "detect-pricing-opportunities",
  auth: "admin",
  rateLimit: { prefix: "detect-pricing", max: 15, windowMs: 60_000 },
}, async ({ supabaseAdmin, body, corsHeaders }) => {
  const { mode = 'full' } = (body || {}) as any;
...
serve(async (req) => {
  ...
  for (const alert of standardAlerts) {
    ...
    if (!existing || existing.length === 0) {
      const { error: insertError } = await supabaseAdmin.from('pricing_alerts').insert(alert);
  ...
  "summary": "résumé stratégique global"
...

      {
        ...
        if (aiInsights.length > 0) {
          ...
          await supabaseAdmin.from('pricing_insights').delete().eq('status', 'new');
          ...
          await supabaseAdmin.from('pricing_insights').insert(rows);
        }
      }
    } catch (aiError) {
