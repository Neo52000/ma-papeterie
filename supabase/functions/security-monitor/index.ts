...
serve(async (req) => {
  ...
  const rlKey = getRateLimitKey(req, "security-monitor");
  if (!(await checkRateLimit(rlKey, 5, 60_000))) {
    return rateLimitResponse(corsHeaders);
  }

  try {
    // Auth : cron secret uniquement
    ...
    const { data: pendingGdpr } = await supabase
      .from("gdpr_requests")
    ...
    return new Response(
      JSON.stringify({ error: "Erreur du moniteur de sécurité" }),
