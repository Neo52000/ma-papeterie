...
Deno.serve(createHandler({
  name: "auto-purchase-orders",
  auth: "admin",
  rateLimit: { prefix: "auto-purchase-orders", max: 10, windowMs: 60_000 },
}, async ({ supabaseAdmin, body }) => {
  const startTime = Date.now();
  const { dry_run = true } = (body || {}) as any;
  ...
  if (!dry_run) {
    for (const [supplierId, orderData] of ordersBySupplier) {
      ...
      const { data: orderNumber } = await supabaseAdmin.rpc("generate_purchase_order_number");
