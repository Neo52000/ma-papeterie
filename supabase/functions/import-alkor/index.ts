...
Deno.serve(createHandler({
  name: "import-alkor",
  auth: "admin",
  rateLimit: { prefix: "import-alkor", max: 5, windowMs: 60_000 },
}, async ({ supabaseAdmin, req, corsHeaders }) => {
  // Reject oversized payloads (max 50MB)
  const contentLength = parseInt(req.headers.get('content-length') || '0', 10);
  if (contentLength > 50 * 1024 * 1024) {
    return jsonResponse(
      { error: 'Payload trop volumineux (max 50 MB)' },
      413,
      corsHeaders,
    );
  }
  ...
  for (const row of rows) {
    ...
    if (alkorSupplierId && famille) {
      categoryId = await resolveCategory(supabaseAdmin, alkorSupplierId, famille, sousFamille);
    }
    ...
    try {
      ...
      if (ean) {
        ...
        if (existingIds && existingIds.length > 0) {
          ...
          const { data: inserted, error } = await withRetry(() => supabaseAdmin
            .from('products')
            .insert(productData)
            .select('id')
            .single());
      ...
      if (savedProductIds.length > 0 && alkorSupplierId && famille && categoryId) {
        await createUnverifiedMapping(supabaseAdmin, alkorSupplierId, famille, sousFamille, categoryId);
      }
  ...
  await flushBatch(supabaseAdmin, 'supplier_offers', alkorOffersBatch, {
