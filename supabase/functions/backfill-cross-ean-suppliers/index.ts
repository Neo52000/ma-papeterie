...
Deno.serve(createHandler({
  name: "backfill-cross-ean-suppliers",
  auth: "admin-or-secret",
}, async ({ supabaseAdmin, body }) => {
  const { dry_run: dryRun = false } = (body || {}) as any;
  ...
  if (duplicateEans.size === 0) {
    return { ok: true, stats, message: "Aucun EAN dupliqué trouvé" };
  }
  ...
  const supplierData = new Map<string, { supplier_id: string; supplier_reference: string | null; supplier_price: number; source_type: string | null }>();
  ...
  for (let i = 0; i < productIdArray.length; i += BATCH_SIZE) {
    ...
    const { data: spRows, error } = await supabaseAdmin
  ...
  if (!dryRun && touchedProductIds.size > 0) {
    ...
    try {
      ...
      for (const pid of pids) {
        try {
          await supabaseAdmin.rpc("recompute_product_rollups", { p_product_id: pid });
