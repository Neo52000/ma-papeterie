...
Deno.serve(createHandler({
  name: "optimize-reorder",
  auth: "admin",
  rateLimit: { prefix: "optimize-reorder", max: 15, windowMs: 60_000 },
}, async ({ supabaseAdmin, body }) => {
  const { mode = "analyze" } = (body ?? {}) as Record<string, string>;
  ...
  if (recentSales) {
    ...
    for (const [_pid, v] of Object.entries(salesVelocity)) {
  ...
  const analysisData = products
    ...
    .filter(
      (p) => p.salesVelocity.estimatedDaysLeft < 30 || p.currentStock <= p.minStockAlert * 1.5,
    );
...
Réponds UNIQUEMENT en JSON valide avec cette structure exacte :
{
  ...
  if (mode === "save" && parsed.recommendations?.length > 0) {
    await supabaseAdmin.from("reorder_suggestions").delete().eq("status", "pending");
