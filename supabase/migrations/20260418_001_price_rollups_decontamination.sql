-- Décontamination des prix : séparer le prix facturé (price_ttc/price_ht) du prix
-- public constaté (public_price_ttc) pour éviter la surfacturation observée sur
-- le produit Liderpapel EAN 8423473806382 (price=9.12 contaminé par le RRP ALSO
-- alors que le vrai price_ttc était 6.84).
--
-- Deux fonctions modifiées :
--   1. recompute_product_rollups : ne touche plus à price_ttc / price_ht.
--      Alimente uniquement public_price_ttc (+ source / updated_at).
--   2. get_catalog_page : retourne price_ttc (facturé) au lieu du COALESCE
--      avec public_price_ttc. Les filtres min/max prix utilisent aussi price_ttc.

-- 1. recompute_product_rollups : isoler v_public_price dans public_price_ttc
...
BEGIN
  ...
  -- price_ttc / price_ht ne sont plus écrasés : ils restent la source de vérité
  -- du prix facturé (saisie admin ou imports fournisseurs via price_ht/ttc).
  -- public_price_ttc reste informatif (prix constaté chez concurrents / PVP).
...
-- 2. get_catalog_page : retourner price_ttc (facturé) au lieu du COALESCE avec
-- public_price_ttc. Pareil pour les filtres min/max price.
