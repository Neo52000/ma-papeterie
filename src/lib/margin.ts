...
/** Calcule la marge sur prix de vente : (pv − pa) / pv × 100 */
...
/** Vérifie si la marge est supérieure ou égale au minimum */
export function isMarginValid(
  priceHt: number,
  costPrice: number,
  minMargin: number = MINIMUM_MARGIN_PERCENT,
): boolean {
  return calculateMargin(priceHt, costPrice) >= minMargin;
}
